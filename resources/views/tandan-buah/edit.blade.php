<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 rounded bg-white mt-10 py-4">
        <header class=" flex justify-center">
            <h3 class=" text-lg font-semibold text-gray-800">Edit Data Tandan Buah PTPN IV GUNUNG BANYU</h3>
        </header>
        <form class="grid gap-5 mt-5 w-1/2" method="POST" action="/tandan-buah/{{$tandan->id}}">
            @csrf
            @method('PUT')
            <x-form.text value="{{$tandan->panen_masuk}}"   label="Panen Masuk" name="panen_masuk"
            placeholder="Masukkan Panen Masuk (Kg)"  ></x-form.text>
            <x-form.text value="{{$tandan->tbs_diolah}}"   label="Tbs Diolah" name="tbs_diolah"
            placeholder="Masukkan Tbs Diolah (Kg)"   ></x-form.text>
            <div class="grid gap-6  md:grid-cols-3">
                <div id="datepicker" class="p-4 bg-white rounded shadow-lg">
                    <input type="text" value="{{$tandan->tanggal}}" id="datepicker-input" name="tanggal" class="w-full h-10 px-2 border rounded" placeholder="Select Date and Time">
                </div>
                @error('tanggal')
